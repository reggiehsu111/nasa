import * as tslib_1 from "tslib";
import { encodeUTF8, encodeBase64 } from "./encode";
import atob from "./atob";
export function hmac(key, message) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const importParams = { name: "HMAC", hash: { name: "SHA-256" } };
        const encodedMessage = encodeUTF8(message);
        const encodedKey = encodeUTF8(atob(key));
        const cryptoKey = yield window.crypto.subtle.importKey("raw", encodedKey, importParams, false, [
            "sign"
        ]);
        const signature = yield window.crypto.subtle.sign(importParams, cryptoKey, encodedMessage);
        return encodeBase64(signature);
    });
}
//# sourceMappingURL=hmac.browser.js.map